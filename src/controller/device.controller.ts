...
export const deviceAddClusterHandler = async (req : Request , res : Response , next : NextFunction) =>{
    ...
    let foundRole = device[0].clusters.find( (ea?) => ea._id == req.body.clusterId)
...
export const deviceRemoveClusterHandler = async (req : Request , res : Response , next : NextFunction) =>{
    let device = await getDevice({_id : req.body.deviceId})
...
export const changeDeviceModeHandler = async (req : Request , res : Response , next : NextFunction) =>{
    let foundDevice = await getDevice(req.query)
    console.log('wk')
    if(!foundDevice[0]){
        return  next(new Error ('Device not found'))
