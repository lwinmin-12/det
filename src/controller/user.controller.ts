...
export const userAddDeviceHandler = async (req : Request , res : Response , next : NextFunction)=>{
  try {
    ...
    let foundRole = user[0].userDevice.find((ea : any) => ea._id ==  req.body.deviceId) 
    if(foundRole){
...
export const userRemoveDeviceHandler = async (req : Request , res : Response , next : NextFunction)=>{
  try {
   ...
   let result = await userRemoveDevice(req.body.user[0]._id , req.body.deviceId)
