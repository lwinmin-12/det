...
export const deviceAddCluster = async (
  deviceId: deviceDocument["_id"],
  clusterId: clusterDocument["_id"]
) => {
  try {
...
export const deviceRemoveCluster = async (
  deviceId: deviceDocument["_id"],
  clusterId: clusterDocument["_id"]
) => {
  try {
...
export const updateDeviceBySerialNo = async (
  serialNo: deviceDocument["serialNo"],
  body: {},
  income: string
) => {
  try {
...
export const deviceAddUser = async (
  deviceId: deviceDocument["_id"],
  userId: UserDocument["_id"]
) => {
  try {
    await deviceModel.findByIdAndUpdate(deviceId, {
      $push: { deviceOwner: userId },
    });
    return await deviceModel.findById(deviceId);
  } catch (e) {}
};

export const deviceRemoveUser = async (
  deviceId: deviceDocument["_id"],
  userId: UserDocument["_id"]
) => {
  try {
    await deviceModel.findByIdAndUpdate(deviceId, {
      $pull: { deviceOwner: userId },
    });
    return await deviceModel.findById(deviceId);
  } catch (e) {}
};
